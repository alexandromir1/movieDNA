    ...
    <div className="mt-5 w-full rounded-[12px] border border-white/[0.09] bg-white/[0.03] px-4 py-4 text-center">
      {showStreak && streak > 0 ? (
        <p className="text-sm text-white">
          <span aria-hidden>🔥</span> Серия:{" "}
          <span className="font-semibold tabular-nums">{streak}</span>
          {streak === 1 ? " день" : streak < 5 ? " дня" : " дней"}
        </p>
      ) : (
        <p className="text-sm text-white/80">Завтра — новый Daily Challenge</p>
      )}
      <p className="mt-1.5 text-xs leading-relaxed text-white/40">
        {showStreak && streak > 0
          ? "Вернись завтра, чтобы не потерять серию."
          : "Один Challenge в день. Увидимся завтра."}
      </p>
      <Link
